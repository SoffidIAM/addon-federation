...
public class DummySamlRequestContext implements
	...
	public DateTime getInboundSAMLMessageIssueInstant() {
		return new DateTime(token.created);
	}
	...
	public String getPeerEntityId() {
		return token.getRequest().getFederationMember().getPublicId();
	}

	public EntityDescriptor getPeerEntityMetadata() {
		MetadataProvider p = getMetadataProvider();
		if ( p == null) return null;
		else
			try {
				return p.getEntityDescriptor(getPeerEntityId());
